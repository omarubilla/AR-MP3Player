...
<body>
  ...
  <script>
    ...
    pointRightGesture.addDirection(fp.Finger.Pinky, fp.FingerDirection.HorizontalRight, 1.0);
  	...
  	// After recognizing a gesture, wait 1 second before recognizing any more gestures.
  	let cooldownTime = 1.0;
      ...
      const estimateHands = async () => {
        ...
        for(let i = 0; i < predictions.length; i++) {
          ...
          if(est.gestures.length > 0) {
            ...
            /* Check what the current gesture is, and call the corresponsing API function */

            if (cooldownTime <= 0) {

            	resultLayer.innerText = gestureStrings[result.name];

            	if (gestureStrings[result.name].localeCompare("Play/Pause") == 0) {
            		playOrPause();
            		cooldownTime = 1.0;
            	}
            	if (gestureStrings[result.name].localeCompare("Vol up") == 0) {
            		volumeUp();
            		cooldownTime = 1.0;
            	}
            	if (gestureStrings[result.name].localeCompare("Vol down") == 0) {
            		volumeDown();
            		cooldownTime = 1.0;
            	}
            	if (gestureStrings[result.name].localeCompare("Prev track") == 0) {
            		previousTrack();
            		cooldownTime = 1.0;
            	}
            	if (gestureStrings[result.name].localeCompare("Next track") == 0) {
            		nextTrack();
            		cooldownTime = 1.0;
            	}
